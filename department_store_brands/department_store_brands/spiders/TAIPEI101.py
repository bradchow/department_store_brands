...
def update_data(data, brand_name, mall, floor, url):
...
if __name__ == "__main__":
    api_url = "https://ws.taipei-101.com.tw/api/v1/shopping/search?lang=zh-tw"
    data = {}
    ...
    for item in json_data:
        ...
        if re.match(r'\d+F(,\d+F)*$', floor):
            ...
            for floor in multiple_floors:
                update_data(data, brand_name=name, mall=mall, floor=floor, url=brand_url)
        else:
            update_data(data, brand_name=name, mall=mall, floor=floor, url=brand_url)
